...
These types of questions are regularly discussed in Singapore, whether in the news or our local coffeeshops. We may hear about the numbers in the most recent year, say in 2017, and some other specific year, say in 2000. However, this means we usually miss out on what's happened in all the other years (like before 2000, and from 2001--2016). We may also hear about different numbers from different people, which can be quite frustrating and confusing.
...
It is interesting to note that the percentage of non-residents in the total population was just under 20\% from 1998 to 2006, but increased rapidly to `r paste(round(df_2$nr[df_2$year==2009], digits = 1), "%", sep = "")` in 2009. From 2006 to 2009, the total population increased by `r format(signif(df$total_pop[df$year==2009] - df$total_pop[df$year==2006], digits = 3), big.mark = ",")` people, of which `r paste(round(((df$nonresident_pop[df_2$year==2009] - df$nonresident_pop[df_2$year==2006]) / (df$total_pop[df$year==2009] - df$total_pop[df$year==2006]))*100, digits = 1), "%", sep = "")` were non-residents, `r paste(round(((df$pr_pop[df_2$year==2009] - df$pr_pop[df_2$year==2006]) / (df$total_pop[df$year==2009] - df$total_pop[df$year==2006]))*100, digits = 1), "%", sep = "")`  were permanent residents, and `r paste(round(((df$sgcitizen_pop[df_2$year==2009] - df$sgcitizen_pop[df_2$year==2006]) / (df$total_pop[df$year==2009] - df$total_pop[df$year==2006]))*100, digits = 1), "%", sep = "")` were Singapore citizens. As a result, the percentage of Singapore citizens in the total population decreased  from `r paste(round(df_2$sg[df_2$year==2006], digits = 1), "%", sep = "")` in 2006 to `r paste(round(df_2$sg[df_2$year==2009], digits = 1), "%", sep = "")` in 2009. 

In 2011, the Singapore government announced it would review the immigration policies in response to concerns from Singapore citizens^[http://www.telegraph.co.uk/expat/expatnews/8502700/Singapore-could-look-to-review-immigration-policies.html]. 